...
from utils import *
from plots import rectangle_YOLO, plot_barplot, plot_den_hist, plot_snsplot
...
def main(main_project_path=None):
    ...
    for i in range(len(list_files_from_labels)):
        if not check_yolo(join_path(path_label, list_files_from_labels[i])):
            count += 1
            df_yolo_ex2 = df_merged_ex1.drop(df_merged_ex1[df_merged_ex1['index_sorted'] == i].index)
    if not count:
        print("Tots els fitxers compleixen amb el format YOLO")

    print("\n######################### Exercici 3 #########################\n")
    for i in range(3):
        Process(target=rectangle_YOLO, args=(
            join_path(path_image, list_files_from_images[i]),
            df_merged_ex1[df_merged_ex1['index_sorted'] == i])).start()
        # time.sleep(1)
    print("[!] S'han obert les 3 primeres imatges (ordenades alfabèticament) amb els seu respectius rectangles\n")
    print("[!] S'ha fet ús de multiprocessament per continuar amb l'execució sense tancar les imatges prèviament\n")
    ...
    Process(target=plot_barplot,
            args=(df_grouped_name, 'name', 'count', "Número total d'objectes de cada clase")).start()
    ...
    Process(target=plot_den_hist,
            args=(top_5[['name', 'name_image']], "name")).start()
    ...
    print("[+] Resposta: En aquest exercici, hem agafat el número d'ocurrències total de cada objecte i "
          ...
          "\n{0}\nAixò vol dir que, per exemple, l'objecte 'car' apareixerà ~7 vegades per imatge".format(
        df_grouped_name))
    ...
    print("\n######################### Exercici 5 #########################\n")
    df_ex5 = df_ex4.copy()
    df_ex5.loc[:,'year'] = replace_column(df_ex5, 'name_image', '(\w+_\d+_\w+-\d+-)|(.png|.txt)', '')
    ...
    title_ex5 = 'El número de cotxes per any, per cada ciutat'
    Process(target=plot_snsplot,
            args=(df_ex5_group_filter, 'year', 'count', 'city', 'bar', title_ex5)).start()
    ...
    df_ex7['year'] = replace_column(df_ex7, 'name_image', '(\w+_\d+_\w+-\d+-)|(.png|.txt)', '')
    df_ex7['is_city'] = True
